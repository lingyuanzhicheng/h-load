// Package main provides the entry point for the H-Load proxy server
package main
...
import (
	...
	"h-load/internal/app"
	"h-load/internal/commands"
	"h-load/internal/container"
...
func runCommand() {
	...
	default:
		...
		fmt.Println("Run 'h-load help' for usage.")
		os.Exit(1)
	}
}
...
func printHelp() {
	fmt.Println("H-Load - Multi-channel AI proxy with intelligent key rotation.")
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  h-load                    Start the proxy server")
	fmt.Println("  h-load <command> [args]   Execute a command")
	fmt.Println()
	fmt.Println("Available Commands:")
	fmt.Println("  migrate-keys    Migrate encryption keys")
	fmt.Println("  help            Display this help message")
	fmt.Println()
	fmt.Println("Use 'h-load <command> --help' for more information about a command.")
}
