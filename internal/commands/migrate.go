...
func RunMigrateKeys(args []string) {
	...
	migrateCmd.Usage = func() {
		fmt.Println("H-Load Key Migration Tool")
		fmt.Println()
		fmt.Println("Usage:")
		fmt.Println("  Enable encryption: h-load migrate-keys --to new-key")
		fmt.Println("  Disable encryption: h-load migrate-keys --from old-key")
		fmt.Println("  Change key: h-load migrate-keys --from old-key --to new-key")
		fmt.Println()
